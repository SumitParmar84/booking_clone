...
const HotelDetails = () => {
  ...
  const bookHotel = async (id) => {
    ...
    const data = await result.json();
    notifications.show({
      title: <Text fw={500}>{data.message}</Text>,
      message: (
        <>
          <Title c="dark" fw={500}>{data.booking.hotel.name}</Title>
          <Title order={2} c="dark" fw={500}>{data.booking.hotel.location}</Title>
        </>
      ),
    ...
    <Container>
      ...
      <Stack>
        ...
        <Flex justify='space-between' px='xl' align='center'>
          ...
          <Button bg="blue" size='lg' onClick={() => bookHotel(hotel._id)}>Book</Button>
        </Flex>
      </Stack>
      <Space h={30} />
