import { Badge, Box, Button, Container, Flex, Grid, Group, Image, Paper, Rating, Slider, Space, Stack, Text, Title, } from '@mantine/core';
...
const HotelSearch = () => {
  ...
  // const [valuePrice, setValuePrice] = useState([1000, 5000]);
  ...
  return (
    <Container>
        ...
        <Grid.Col span={8}>
          <Stack>
            {hotels.slice(0, 5).map((e) => {
              1
              return (
                <Paper p={20} key={e._id} c='dark' shadow='sm' withBorder component={Link} to={`/hotel/${e._id}`}>
                  <Flex gap={20}>
                    <Box>
      ...
      <Flex justify='space-between'>
        ...
        <Button disabled={page === 6} variant='subtle' onClick={() => setPage(page + 1)}>Next</Button>
      </Flex>
      <Space h={30} />
    </Container>
  )
