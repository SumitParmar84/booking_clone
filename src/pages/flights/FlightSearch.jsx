...
const FlightSearch = () => {
  ...
  useEffect(() => {
    ...
    fetchFlights()
  }, [from, page, params.day, params.from, params.to, stop, to, valuePrice,valueDuration])
  ...
  const bookFlight = async (id) => {
    ...
    console.log(data);
    notifications.show({
      title: <Text fw={400}>{data.message}</Text>,
      message: (
        <>
    ...
    {from !== to ? (
      <Container>
                      ...
                      <Paper key={e._id} shadow='sm' p={rem(40)}>
                        <Flex gap={20} justify="space-around" align="center">
                          ...
                          <Box>
                            ...
                            <Text ta='center'>Stops : {e.stops}</Text>
                          </Box>
                          <Stack>
        ...
        <Flex justify='space-between'>
          ...
          <Button disabled={page === 7} variant='transparent' onClick={() => setPage(page + 1)}>Next</Button>
        </Flex>
        <Space h={30} />
      </Container>
    ) : (
